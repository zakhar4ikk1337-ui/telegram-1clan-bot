...
promo_codes["30days"].remove(code)
used_codes.append(code)
...
await update.message.reply_text(f"Использованные:\n{used_codes}")
...
key = "7days" if days == 7 else "30days"

if not promo_codes[key]:
await context.bot.send_message(
user.id,
"❌ Нет промокодов, обратитесь к админу"
)
return

code = promo_codes[key].pop(0)
used_codes.append(code)
